document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("BGS Draft Custom Board Loaded");

    const selectors = {
        model: 'input[name="cdr-model"]',
        construction: 'input[name="cdr-construction"]',
        length: "#text-1",
        width: "#text-2",
        thickness: "#text-3",
        volume: "#text-4"
    ...
    function getSelectedModel() {
        return document.querySelector('input[name="cdr-model"]:checked')?.value ?? null;
    }

    function getSelectedConstruction() {
        return document.querySelector('input[name="cdr-construction"]:checked')?.value ?? null;
    }
    ...
    function updateBasePrice() {
        ...
        const option = document.querySelector(
            `input[name="cdr-baseprice"][value="${value}"]`
        );
    ...
    if (lengthInput) {
        ...
        if (e.target.matches('input[name="cdr-construction"]')) {
